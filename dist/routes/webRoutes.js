...
const requireAlumni = (0, auth_1.requireRole)("alumni");
...
function handleUnauthorized(req, res, response) {
...
router.get("/profile", requireAlumni, async (req, res, next) => {
    try {
        ...
        try {
            ...
            if (profileRow.rows[0]) {
                ...
                const bidResult = await query("SELECT id, amount, bid_month, is_winner FROM monthly_bids WHERE alumni_id = $1 AND bid_month = $2 ORDER BY created_at DESC LIMIT 1", [alumniId, bidMonth]);
...
router.post("/profile", requireAlumni, async (req, res, next) => {
    try {
        const response = await (0, undici_1.fetch)(`${apiUrl(req)}/profile`, {
            method: "PUT",
...
router.post("/profile/image", requireAlumni, upload.single("image"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        await query(`INSERT INTO alumni_profiles (user_id, profile_image_url, updated_at)
       VALUES ($1, $2, NOW())
       ON CONFLICT (user_id)
       DO UPDATE SET profile_image_url = EXCLUDED.profile_image_url, updated_at = NOW()`, [req.session.userId, profileImageUrl]);
...
// ─── Monthly bid ──────────────────────────────────────────────────────────────
router.get("/profile/bid", requireAlumni, async (req, res, next) => {
    try {
        const { query } = await Promise.resolve().then(() => __importStar(require("../config/db")));
        ...
        const [currentBidResult, historyResult, countResult] = await Promise.all([
            query("SELECT id, amount, bid_month, is_winner, created_at FROM monthly_bids WHERE alumni_id = $1 AND bid_month = $2 ORDER BY created_at DESC LIMIT 1", [alumniId, bidMonth]),
            query("SELECT id, amount, bid_month, is_winner, created_at FROM monthly_bids WHERE alumni_id = $1 ORDER BY created_at DESC", [alumniId]),
            query("SELECT COUNT(*) AS cnt FROM monthly_bids WHERE alumni_id = $1 AND bid_month = $2", [alumniId, bidMonth]),
        ]);
...
router.post("/profile/bid", requireAlumni, async (req, res, next) => {
    try {
        const { query } = await Promise.resolve().then(() => __importStar(require("../config/db")));
        const userId = req.session.userId;
        ...
        const countResult = await query("SELECT COUNT(*) AS cnt FROM monthly_bids WHERE alumni_id = $1 AND bid_month = $2", [alumniId, bidMonth]);
        ...
        const existingResult = await query("SELECT id, amount FROM monthly_bids WHERE alumni_id = $1 AND bid_month = $2 ORDER BY created_at DESC LIMIT 1", [alumniId, bidMonth]);
        ...
        if (existing) {
            ...
            await query("UPDATE monthly_bids SET amount = $1, updated_at = NOW() WHERE id = $2", [amount, existing.id]);
        }
        else {
            // Insert a new bid row
            await query("INSERT INTO monthly_bids (alumni_id, amount, bid_month) VALUES ($1, $2, $3)", [alumniId, amount, bidMonth]);
        }
...
sections.forEach((section) => {
    const route = section === "professional_courses" ? "courses" :
        section === "employment_history" ? "employment" :
            section;
    router.get(`/profile/${route}`, requireAlumni, async (req, res, next) => {
        try {
    ...
    router.post(`/profile/${route}`, requireAlumni, async (req, res, next) => {
        try {
    ...
    router.post(`/profile/${route}/:id/delete`, requireAlumni, async (req, res, next) => {
        try {
            const apiHeaders = {
                ...toCookieHeader(req),

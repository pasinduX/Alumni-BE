...
router.get(
  ...
  async (req: ApiKeyRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { programme, graduation_year, industry_sector } = req.query as Record<string, string | undefined>;
...
router.get(
  ...
  async (req: ApiKeyRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { programme, graduation_year, industry_sector } =
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const [degrees, certifications, licences, professionalCourses, employmentHistory] =
        await Promise.all([
          query(
            `SELECT id, title, institution, url, completed_at
             FROM degrees
             WHERE user_id = $1
          ...
          query(
            `SELECT id, title, provider, url, completed_at
             FROM certifications
             WHERE user_id = $1
             ORDER BY completed_at DESC NULLS LAST`,
            [id]
          ...
          query(
            `SELECT id, title, awarding_body, url, completed_at
             FROM licences
             WHERE user_id = $1
             ORDER BY completed_at DESC NULLS LAST`,
            [id]
